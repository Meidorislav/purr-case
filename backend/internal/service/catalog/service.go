...
const customAttributesField = "custom_attributes"
...
func (s *Service) GetCatalogItems(ctx context.Context, token string) ([]dto.Item, error) {
	...
	for _, itemType := range itemTypes {
		result, err := s.FetchItems(ctx, token, itemType, customAttributesQuery)
		if err != nil {
			return nil, err
		}
...
func additionalFieldsQuery(fields ...string) string {
	values := url.Values{}
	for _, field := range fields {
		if field == "" {
			continue
		}
		values.Add("additional_fields[]", field)
	}
